...
const Moodcomponet = ({ mood ,id , image }) => {
  return (
    <Link to = {`/mood/${mood}/${id}`} className="w-[180px]  hover:shadow-sm hover:shadow-purple-500 mt-4  m-2 p-1 flex flex-col justify-center items-center pt-4 h-full bg-[#1F1F1F] ">
      <div className="w-[150px]  h-[190px]  overflow-hidden relative ">
      ...
      <div className="flex justify-between  m-auto w-[90%] mt-4 mb-3 items-center">
        <div className=" w-[100px] h-[30px] flex justify-center items-center ">
          <h3 className="text-white capitalize whitespace-nowrap overflow-hidden text-[15px] ">
            {mood}
          </h3>
        </div>
        <div>
          <PiPlaylist className="" color="purple" size={"27px"} />
        </div>
      </div>
    </Link>
  );
